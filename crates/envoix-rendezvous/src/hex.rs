...
mod tests {
    ...
    fn parse_round_trip() {
        ...
        let bytes = parse_hex::<16>(input).expect("valid");
    ...
    fn parse_rejects_wrong_length() {
        assert!(parse_hex::<16>("").is_none());
        assert!(parse_hex::<16>("ab").is_none());
        assert!(parse_hex::<16>(&"0".repeat(31)).is_none());
        assert!(parse_hex::<16>(&"0".repeat(33)).is_none());
    }
    ...
    fn parse_rejects_non_hex() {
        assert!(parse_hex::<16>("0123456789abcdef0123456789abcdeg").is_none());
        assert!(parse_hex::<16>("zzzzzzzzzzzzzzzzzzzzzzzzzzzzzzzz").is_none());
    }
}
